...
$(function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            console.log(data);
            if(data.data.details[0].services.length===1){
                $(".details").hide();
            }
          ...
          // 服务详情点击
            for (let i = 0; i < $(".details>button").length; i++) {
                $(".details>button").eq(i).on("click", function () {
                    ...
                    $(".details-head p .price").text(data.data.details[i].salePrice)
                })
            }
            // 不能自理判断图片
            ...
            $(".immediately").on("click", function () {
                for (let i = 0; i < $(".day-nurse button").length; i++) {
                    if ($(".day-nurse button").eq(i).hasClass("active")) {
                        window.location.href = "buy-order(time).html?id=" + data.data.id + "&name=" + data.data.name + "&type=" + (i+1);
                    }
                }
